...
class ViewController: UIViewController {
  ...
  func setUpCamera() {
    ...
    videoCapture.setUp(sessionPreset: AVCaptureSession.Preset.hd1280x720) { success in
      if success {
        ...
        self.boundingBoxes.addToLayer(self.videoView.layer)
  ...
  func predict(image: UIImage) {
    if let pixelBuffer = image.pixelBuffer(width: SNACKMODEL.inputWidth, height: SNACKMODEL.inputHeight) {
  ...
  func showOnMainThread(_ boundingBoxes: SNACKMODEL.Prediction, _ elapsed: CFTimeInterval) {
    if drawBoundingBoxes {
      DispatchQueue.main.async {
        ...
        self.resultLabel.text = String(format: "Elapsed %.5f seconds - %.2f FPS", elapsed, fps)
      }
    }
  }
  ...
  func show(predictions: SNACKMODEL.Prediction) {
    ...
    let scaleX = width / CGFloat(SNACKMODEL.inputWidth)
    let scaleY = height / CGFloat(SNACKMODEL.inputHeight)
